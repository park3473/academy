                                ...
                                <div class="member_input_wrap">
                                    <ul class="member_input">
                                        ...
                                        <li>
											...
											<img style="width:100%;height:100%;object-fit:cover" id="preview_img" src="${pageContext.request.contextPath }/resources/upload/program/image/${model.view.image}" alt="no"/>
											</div>
...
$(document).ready(function () {
	...
	$(".adm_menu_con > li").eq(3).css({
	 ...
	 // JSTL 변수를 JavaScript 변수로 변환
    const tagList = [
        <c:forEach var="tag" items="${model.taglist}" varStatus="loop">
            {
                pro_idx: ${tag.pro_idx},
                name: "${tag.name}",
                tag_idx: ${tag.tag_idx},
    ...
    // 모든 checkbox를 초기 상태(unchecked)로 설정
    $("input[type=checkbox][name=tag]").prop('checked', false);

    // tagList를 반복하여 일치하는 체크박스를 찾아서 checked 상태로 설정
